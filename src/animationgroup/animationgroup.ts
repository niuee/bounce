...
export class AnimationGroupB{
    ...
    animate(deltaTime: number){
        if(this.localTime <= this.duration && this.onGoing){
            ...
            for(let index = 0; index < this.keyframesList.length; index++){
                const animationSequence = this.keyframesList[index];
                if(animationSequence.startTime != undefined && animationSequence.startTime > this.localTime){
                    continue;
                }
                ...
                let localDeltaTimePercentage = (this.localTime - animationSequence.startTime) / animationSequence.duration;
